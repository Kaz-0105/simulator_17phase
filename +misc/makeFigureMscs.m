...
for file_name = file_names
    ...
    for num_phases = unique(results.num_phases)'
        PhaseDataMap(num_phases) = struct(...
            'relflow_id', results.relflow1(results.num_phases == num_phases), ...
    ...
    % 信号待ちの車列の結果のプロット
    figure;
    bar([PhaseDataMap(4).queue, PhaseDataMap(8).queue, PhaseDataMap(17).queue]);
    xticklabels(relflow_labels(PhaseDataMap(4).relflow_id));
    ...
    % 遅れ時間の結果のプロット
    figure;
    bar([PhaseDataMap(4).delay, PhaseDataMap(8).delay, PhaseDataMap(17).delay]);
    xticklabels(relflow_labels(PhaseDataMap(4).relflow_id));
    xlabel('Route Selection Type', 'FontSize', LABEL_FONT_SIZE);
